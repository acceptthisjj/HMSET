function U = featureExtract(X,Y,method,type_num)
...
%         U = pca(X);%由于需要使用matlab自带的pca 改名
    ...
    case 'lpdp_u'
        ...
        options.t = method.t;
        A = constructA(X,options); 
        U = lpdp_u(X,Y,A,method.mu,type_num,method.gamma,method.ratio_b,method.ratio_w);
    otherwise
